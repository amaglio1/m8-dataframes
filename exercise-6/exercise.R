...
MostPopular <- function(my.year) {
  result.prop <- prop[year == my.year]
  result.names <- name[year == my.year]
  result.name <- result.names[result.prop == max(result.prop)]
  result <- paste('The most popular name was', result.name)
  return(result)
}
